...
exp(model$coefficients[2]) #coeff negative, so exp(coeff) less than 0, meaning the odds decrease. Less chance of sucessfully climb mountain the higher it is
exp(model$coefficients[3]) #Same as above

#Test significance with Wald test
standard_error <- sqrt(diag(vcov(model)))
pvalue_coeff1 <- 2*pnorm( (model$coefficients[1] - 0) / standard_error[1]) #significant
pvalue_coeff2 <- 2*pnorm( (model$coefficients[2] - 0) / standard_error[2]) #significant
pvalue_coeff3 <- 2*pnorm( (model$coefficients[3] - 0) / standard_error[3]) #significant
...
exp_confint_coeff2 <- exp(confint_coeff2) #less than 1 with prob 95%, probably negative effect on odds
exp_confint_coeff3 <- exp(confint_coeff3) #Same, but very close to 1. However, only in unit one meter so effect intuitvley not so large
...
pi_everest <- exp(nu_everest)/(1+exp(nu_everest)) #9% prob of being successful
max(mount$height) #everst ish 400 meters above max in data for model fitting
max(mount$prominence) #almost twice as large as max in data. Therefore model is perhaps not as good, as everest is so different from data used to fit model
...
confint_pi_everest <- exp(confint_nu_everest)/(1 + exp(confint_nu_everest)) #quite large, but main reasons described above
...
pi_chogolisa <- exp(nu_chogolisa)/(1+exp(nu_chogolisa)) #70% prob of being successful
...
confint_pi_chogolisa <- exp(confint_nu_chogolisa)/(1 + exp(confint_nu_chogolisa)) #between 63% and 76%
y_chogolisa_hat <- pi_chogolisa*22
confint_y_chogolisa_hat <- confint_pi_chogolisa*22 #actual 20 success, not in 95% confint. 
```


# Part 1: 
In this exercise we will use binary regression with a logit link to model the probability of successfully climbing a mountain given it's height and prominence as covariates. We assume the number of people who successfully climbes mountain $i$ $Y_i$, is binomial distrubuted, $Y_i \sim$ Bin$(n_i, \pi_i)$, where $n_i$ is the number of poeple attempting to climb the mountain, and $\pi_i$ is the probability for success. That is, a model on the form,
...
To arrive at the maximum likelihood estimators for $\beta$, one would find the maximum of the log-likelihood by numerical optimazation. In practice that would mean to find the score funtion, $S(\beta)$, by differentiating $l(\beta)$, and solve
$$
S(\beta) = \frac{\partial l(\beta)}{\partial \beta} = 0.
$$
This is solved by Newton-Raphson or Fisher Scoring algorithm, which for an exponential distribution with the canonical link (like this model) are the same. It is an interative method on the form
$$
...
Using the fact that the parameters are asympotically normally distributed we can construct an 95\% confidence interval by
$$
...
$$
where $\tilde{\pi}$ denote the probability for the saturated model. This probability is simply $\tilde{\pi}_i = y_i / n_i$, such that the estimated number of succsesses is equal to the actual number for each mountain. The deviance for mountain $i$ is 
$$
...
ggplot(data = gg_frame, aes(x = prominence, y=deviance)) + geom_point() + geom_hline(yintercept=0, linetype="dotted", colour="red")
```
It seems that the residuals are reasonably randomly distributed around 0 for both "low" mountains and high mountains. However, it seems that the model underestimates the probability of sucssess for the lowest mountains quite a lot, as there are some very large residuals for the lowest mountains. The residuals for the somewhat larger mountains seems to in general be closer to zero.  As a consequence of the "easy" mountains, it seems that the model often overestimates the probability of success for the more "average" mountains, as there seems to in general be more negative residuals than positive.

For the prominence we see pretty much the same pattern as for the height. The residuals seem to be reasonably randomly distributed around zero, expect for a few outliers for small prominence. It seems that the model also underestimates the prbability of success in this case. For larger prominence the residuals are in general smaller. As with the previous plot it seems to in general be more negative residuals than positive.

### Estimated probabilities as function of the covariates
We proceed by plotting the estimated probability as a function of the covarites height and prominence. We have that
$$
...
```
We see that the estimated probability of successfully climibing mount Everest is quite low, around 9\%. The 95\% confidence interval is rather large, from just over 5\% to 14\%. We note that Mount Everest is rather extreme compared to the other mountains in the data set, being approx. 400 meters higher than the heighest in the data set (more than 1000 above the average), and almost twice as large prominence as the largest in the data set (almost 6 times the average prominence). This means the model does not necessarily describe Mount Everest as good as the other mountains. 
...
```
We see that the estimated number of people successfully climbing the mountain is somewhat smaller than the true number. This is consistent with our observation that the model have a tendency to underestimate the probability for mountains with small prominence, whih is the case here. 
