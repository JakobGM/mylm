...
We calculate the deviance of each mountain with the build in R-function, and plot the deviance against height and prominence:
```{r}
...
We have that $x_{chogolisa, height} = 7665$ and $x_{chogolisa, prominence} = 1624$. We calculate the estimated probability for success, and corresponding 95\% confidence interval, in the same way as for Mount Everest. In addition we calculate the estimated $y_{chogolisa}$ by $\hat y_{chogolisa} = \hat\pi_{chogolosa} n_{chogolisa}$, and the confidence interval.
